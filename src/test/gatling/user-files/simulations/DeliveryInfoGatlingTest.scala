...
/**
 * Performance test for the DeliveryInfo entity.
 */
class DeliveryInfoGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the DeliveryInfo entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all deliveryInfos")
            .get("/order/api/delivery-infos")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new deliveryInfo")
            .post("/order/api/delivery-infos")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "deliveryType":"SAMPLE_TEXT"
                , "expectedDelivery":"2020-01-01T00:00:00.000Z"
                , "deliveryCharge":null
                }""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_deliveryInfo_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created deliveryInfo")
                .get("/order${new_deliveryInfo_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created deliveryInfo")
            .delete("/order${new_deliveryInfo_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }

...
/**
 * Performance test for the AuxilaryOrderLine entity.
 */
class AuxilaryOrderLineGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the AuxilaryOrderLine entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all auxilaryOrderLines")
            .get("/order/api/auxilary-order-lines")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new auxilaryOrderLine")
            .post("/order/api/auxilary-order-lines")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                }""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_auxilaryOrderLine_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created auxilaryOrderLine")
                .get("/order${new_auxilaryOrderLine_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created auxilaryOrderLine")
            .delete("/order${new_auxilaryOrderLine_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
